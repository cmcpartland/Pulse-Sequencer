...
# A class to create a PulseSequence. A PulseSequence contains PulseChannels, and each PulseChannel contains a number of PulseTrains.
# A PulseSequence object has the ability to add, remove, or edit the PulseTrains that belong to a PulseChannel. It can also convert
# the user-created PulseSequence (via the PulseSequenceUI gui) into instructions for the PulseBlaster.
...
class PulseSequence:
    ...
    # Return the first event of the sequence, which is either a wait event or the beginnig of the first pulse(s).
    def getFirstSequenceEvent(self):
        self.setFirstSequenceEvent()
        return self.first_sequence_event
        
    # Deterine the first evnent of the sequence.
    ...
    # a list of PulseBlaster instructions, a list of the same instructions in a readable form, 
    ...
    # number of loops), and whether or not the sequence contains a bad instruction (a boolean value, will be
    # true if the PulseBlaster is not capable of outputting the user-created pulse sequence).
    def convertSequenceToInstructions(self, inf_loop, num_of_loops):
        self.setFirstSequenceEvent()
        self.setLatestSequenceEvent()
